...
export interface HKTBRetainerItem {
  id: string;
  dateFrom: string;
  dateTo: string;
...
export interface HKTBRetainerData {
  invoiceNo: string;
  invoiceDate: string;
  items: HKTBRetainerItem[];
}
...
interface Props {
  data: HKTBRetainerData;
}

export default function HKTBRetainerPdf({ data }: Props) {
          ...
          {data.items.map((item) => {
            ...
            return (
              <View key={item.id} style={s.tableRow}>
                <View style={[s.cell, s.colDate, { alignItems: 'center' }]}>
                  <Text style={s.dateRangeText}>{item.dateFrom}{'\n'}~{'\n'}{item.dateTo}</Text>
                </View>
                <View style={[s.cell, s.colJob, { justifyContent: 'center' }]}>
                  <Text style={{ fontSize: 8, textAlign: 'center' }}>{item.jobDescription}</Text>
                </View>
                <View style={[s.cell, s.colPrice, { alignItems: 'center' }]}>
                  <Text style={{ fontSize: 8 }}>{fmt(price)}</Text>
                </View>
                <View style={[s.cell, s.colVAT, { alignItems: 'center' }]}>
                  <Text style={{ fontSize: 8 }}>{fmt(vat)}</Text>
                </View>
                <View style={[s.cellLast, s.colTotal, { alignItems: 'center' }]}>
                  <Text style={{ fontSize: 8 }}>{fmt(total)}</Text>
                </View>
              </View>
            );
          })}

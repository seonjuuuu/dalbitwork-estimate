...
function makeDefaultData(): HKTBInvoiceData {
  const today = new Date().toISOString().split('T')[0];
  return {
    invoiceNo: `${today.replace(/-/g, '')}001A`,
    invoiceDate: today,
    items: [
      { id: nanoid(), date: '', jobDescription: 'Translation', unitPrice: '2,300', quantity: '813' },
    ],
  };
}
...
export default function HKTBInvoice() {
  ...
  const renderPreview = useCallback(async () => {
    setIsRendering(true);
    try {
      const blob = await pdf(<HKTBInvoicePdf data={data} />).toBlob();
  ...
  const handleLoad = (inv: NonNullable<typeof savedList>[number]) => {
    setData({
      ...
      items: (inv.items as HKTBInvoiceItem[]) ?? [],
    });
    setSavedId(inv.id);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `HKTB-INVOICE-${data.invoiceNo || 'draft'}.pdf`;
      a.click();
  ...
  const updateItem = (id: string, field: keyof HKTBInvoiceItem, value: string) => {
    setData(prev => ({
      ...prev,
  ...
  const addItem = () => {
    setData(prev => ({
      ...prev,
      items: [...prev.items, { id: nanoid(), date: '', jobDescription: '', unitPrice: '', quantity: '' }],
    }));
  };
              ...
              {savedList && savedList.length > 0 ? (
                <div className="p-2 space-y-1">
                  {savedList.map(inv => {
                    ...
                    const dateStr = items[0]?.date?.trim();
                    const periodLabel = dateStr || inv.invoiceDate.slice(0, 7);
                    return (
                      <div
                        key={inv.id}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-lg p-5">
            ...
            <div className="grid grid-cols-[80px_1fr_100px_80px_20px] gap-2 mb-2 px-3">
              ...
              <span className="text-xs font-medium text-muted-foreground text-right">Quantity</span>
              <span />
            </div>
              ...
              {data.items.map((item) => {
                ...
                return (
                  <div key={item.id} className="border border-border rounded-md p-3 space-y-2">
                    <div className="grid grid-cols-[80px_1fr_100px_80px_20px] gap-2 items-center">
                      <Input
                        value={item.date}
                      ...
                      <Input
                        ...
                        onChange={e => updateItem(item.id, 'unitPrice', autoFormatNumber(e.target.value))}
                        placeholder="2,300"
                        className="text-xs h-8 text-right"
                      />
                      <Input
                        value={item.quantity}
                        onChange={e => updateItem(item.id, 'quantity', autoFormatNumber(e.target.value))}
                        placeholder="813"
                        className="text-xs h-8 text-right"
                      />
                      <button

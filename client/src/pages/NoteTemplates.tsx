...
export default function NoteTemplates() {
  ...
  const [formData, setFormData] = useState<TemplateFormData>({ name: '', notes: [''] });
  ...
  const handleCreate = () => {
    ...
    createMutation.mutate({ name: formData.name.trim(), notes: filteredNotes });
  };
  ...
  const handleUpdate = () => {
    ...
    updateMutation.mutate({
      id: editingId,
      data: { name: formData.name.trim(), notes: filteredNotes },
    });
  };
  ...
  const startEdit = (tmpl: { id: number; name: string; notes: string[] }) => {
    setEditingId(tmpl.id);
    setIsCreating(false);
    setFormData({ name: tmpl.name, notes: [...tmpl.notes] });
  };

...
export const appRouter = router({
  ...
  hktbInvoices: router({
    ...
    setAutoReminderEnabled: protectedProcedure
      .input(z.object({ enabled: z.boolean(), monthlyPrice: z.string().optional() }))
      .mutation(async ({ ctx, input }) => {
        return db.setHktbRetainerAutoEnabled(ctx.user.id, input.enabled, input.monthlyPrice);
      }),
  }),
  ...
  ai: router({
    ...
    draftEstimate: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // 가격 정책 관련 고정 안내 문구는 AI에게 맡기지 않고 항상 고정으로 추가 (정책 누락 방지)
        // 제안서(견적 전 단계)와 견적서(확정 단계)는 금액 확정 여부 안내 문구가 다름
        const FIXED_POLICY_NOTES = input.docType === "proposal"
          ? [
          ...
          : [
              "본 견적서의 금액은 확정 금액이며, 추가 작업 발생 시 별도 협의입니다.",
              "아임웹 결제 비용은 고객사 별도 결제 사항입니다.",
              "이미지 및 콘텐츠 소스(텍스트, 로고, 제품사진 등)는 고객사 제공 기준입니다.",
              "매출증빙: 현금영수증 발행",
            ];
        ...
        const notes = items.some(i => i.name === "추가 페이지")
          ? [...draft.notes, "추가 페이지 비용은 페이지당 15만원 기준으로 산정했으며, 실제 내용과 구성에 따라 금액은 변동될 수 있습니다.", ...FIXED_POLICY_NOTES]
          : [...draft.notes, ...FIXED_POLICY_NOTES];
